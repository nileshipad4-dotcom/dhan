...
UNDERLYINGS = {
    ...
    "FINNIFTY": {"scrip": 27, "seg": "IDX_I", "security_id": 260112},
    "MIDCAP NIFTY SELECT": {"scrip": 28, "seg": "IDX_I", "security_id": 260113},
    "SENSEX": {"scrip": 51, "seg": "IDX_I", "security_id": 256777},
}
...
for strike in selected_strikes:
    ...
    rows.append({
        ...
        "CE IV": int(ce["implied_volatility"] * 10000) if ce.get("implied_volatility") is not None else None,
        "CE Delta": int(ce["greeks"]["delta"] * 100000) if ce.get("greeks", {}).get("delta") is not None else None,
        "CE Gamma": int(ce["greeks"]["gamma"] * 10000000) if ce.get("greeks", {}).get("gamma") is not None else None,
        "CE Vega": int(ce["greeks"]["vega"] * 10000) if ce.get("greeks", {}).get("vega") is not None else None,

        "PE LTP": round(pe["last_price"], 2) if pe.get("last_price") else None,
        "PE OI": pe.get("oi"),
        "PE Volume": pe.get("volume"),
        "PE IV": int(pe["implied_volatility"] * 10000) if pe.get("implied_volatility") is not None else None,
        "PE Delta": int(pe["greeks"]["delta"] * 100000) if pe.get("greeks", {}).get("delta") is not None else None,
        "PE Gamma": int(pe["greeks"]["gamma"] * 10000000) if pe.get("greeks", {}).get("gamma") is not None else None,
        "PE Vega": int(pe["greeks"]["vega"] * 10000) if pe.get("greeks", {}).get("vega") is not None else None,
    })
